...
impl AggregatedTranscript {
    ...
    pub fn verify_decryption_share_simple(
        &self,
        dkg: &Dkg,
        share: &DecryptionShareSimple,
        share_index: u32,
        validator_public_key: &ValidatorPublicKey,
        ciphertext: &Ciphertext,
    ) -> Result<bool> {
        let blinded_key_share =
            self.0.aggregate.get_share_for_index_and_pubkey(
                share_index,
                validator_public_key,
            )?;
        // `combine_shares_simple` derives the Lagrange coefficients from the
        // share's own domain point, so it must be the one for this index.
        let expected_domain_point = dkg.0.get_domain_point(share_index)?;
        Ok(share.domain_point == expected_domain_point
            && share.share.verify(
...
mod test_ferveo_api {
    ...
    #[test_case(30, 16; "N is not a power of 2, t is 1 + 50%")]
    #[test_case(30, 30; "N is not a power of 2, t=N")]
    fn verify_decryption_share_simple(
        shares_num: u32,
        security_threshold: u32,
    ) {
        let rng = &mut StdRng::seed_from_u64(0);
        ...
        let ciphertext =
            encrypt(SecretBox::new(MSG.to_vec()), AAD, &aggregate.public_key())
                .unwrap();
        let share = aggregate
            .create_decryption_share_simple(
                &dkg,
        ...
        // A share created by validator `me` verifies against its own index and key
        assert!(aggregate
            .verify_decryption_share_simple(
                &dkg,
                &share,
                me.share_index,
                &me.public_key,
        ...
        // The same share does not verify against another validator's key
        assert!(!aggregate
            .verify_decryption_share_simple(
                &dkg,
                &share,
        ...
        // Nor against another validator's index
        assert!(!aggregate
            .verify_decryption_share_simple(
                &dkg,
                &share,
        ...
        // Nor when attributed to another validator entirely
        assert!(!aggregate
            .verify_decryption_share_simple(
                &dkg,
                &share,
        ...
        // A share created from a different DKG (same validators, different
        // ritual) for the same ciphertext does not verify
        let (other_messages, _, _) = make_test_inputs(
            rng,
            TAU,
            security_threshold,
            shares_num,
        ...
        let other_messages = &other_messages[..shares_num as usize];
        let other_aggregate =
            dkg.aggregate_transcripts(other_messages).unwrap();
        assert_ne!(aggregate.public_key(), other_aggregate.public_key());
        let other_share = other_aggregate
            .create_decryption_share_simple(
                &dkg,
                &ciphertext.header().unwrap(),
                AAD,
        ...
        // A share with a tampered domain point does not verify, even though
        // the underlying tdec share is still valid for this index
        let tampered = DecryptionShareSimple {
            share: share.share.clone(),
            domain_point: dkg.0.get_domain_point(other.share_index).unwrap(),
        };
        assert!(tampered.share.verify(
            &aggregate.0.aggregate.shares[me.share_index as usize],
            &me.public_key.encryption_key,
            &ciphertext.0,
        ));
        assert!(!aggregate
            .verify_decryption_share_simple(
        ...
        // An out-of-range share index is an error, not a `false`
        assert!(matches!(
            aggregate.verify_decryption_share_simple(
                &dkg,
                &share,
                shares_num,
                &me.public_key,
            ...
            Err(Error::InvalidShareIndex(index)) if index == shares_num
        ));
    }
